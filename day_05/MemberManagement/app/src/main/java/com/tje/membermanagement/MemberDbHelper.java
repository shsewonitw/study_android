...
public class MemberDbHelper extends SQLiteOpenHelper {
    ...
    public int count(){
        ...
        String sql = String.format("select count(*) from %s",MemberInfo.TABLE_NAME);
    ...
    public int count(String id){
        SQLiteDatabase db = getReadableDatabase();
        String sql = String.format("select count(*) from %s where id = ?",MemberInfo.TABLE_NAME);
    ...
    public boolean insert(Member member){
        ...
        // return 되는 long 값에 대해 -1이 return 될 경우 error 이며 그렇지 않을경우 삽입된 rowId가 리턴
